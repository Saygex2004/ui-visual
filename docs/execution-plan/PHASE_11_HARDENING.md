# Phase 11 — Hardening: Accessibility, Performance, Polish, Easter Eggs
...
You are building **`pvp-dashboard`**: the web application with which a small investment team reviews Italian judicial auctions — a React + Radix UI SPA over a Fastify + Zod API mediating a shared Cloud Firestore database kept current by a separate scraper project. Every feature exists (Phases 0–10), now on a unified modern-enterprise design system with dark mode. This phase is the difference between "works in the demo path" and "holds up under a keyboard-only user, a 4k-row filter, a narrow window, and a stale deploy" — an audit-and-fix session across the whole application, with the quality gates as the referee.

**State when this phase starts:** Phases 0–10 delivered the complete feature set, the Radix migration, and the modern-enterprise design system with dark mode, per-phase tests green — see `HANDOFF_PHASE_0..10.md`. Cross-cutting audits have not run; the easter eggs do not exist.
...
1. `HANDOFF_PHASE_10.md` outcome `complete`; all suites green at the inherited commit.
...
7. **Easter eggs** (UI Appendix B, constants from `packages/shared`): the > €5M *Vai all'annuncio* interception dialog (proceed/dismiss, `Esc`/outside-click dismisses); the L'Aquila-province and Roccaraso dialogs with their single-action semantics (Roccaraso's action both dismisses and opens). Built on the Radix `Dialog` primitive introduced in Phase 9. Themed media ships bundled; both flows e2e-covered.
8. **Hallmark audit:** run the skill's `audit` verb over the principal surfaces — a confirmatory pass over the design system defined in Phase 10, in **both light and dark themes**; triage the punch list — fix what violates the token/anti-slop disciplines, record consciously-kept items.
