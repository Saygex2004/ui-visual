// Integration — listings module, HTTP layer (TESTING.md §3/§4). No auth in
// this phase (Phase 3 locks every route behind sessions).
...
describe('listings module (HTTP, over the emulator)', () => {
  ...
  it('GET /readyz reports ready once the data layer is primed', async () => {
  ...
  it('ETag/If-None-Match: repeating the request with the returned ETag answers 304, empty body', async () => {
    const first = await app.inject({ method: 'GET', url: '/api/areas/immobili/snapshot' });
  ...
  it('a stale If-None-Match still returns 200 with the full snapshot', async () => {
    const res = await app.inject({
      method: 'GET',
      url: '/api/areas/immobili/snapshot',
      headers: { 'if-none-match': 'not-a-real-version' },
    });
    expect(res.statusCode).toBe(200);
  });

  it('GET /api/areas/:area/snapshot 404s for an unknown area slug', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/areas/pippo/snapshot' });
  ...
  it('GET /api/listings/:id returns the full untrimmed descrizione and correct classification', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/listings/1055' });
    expect(res.statusCode).toBe(200);
    const body = res.json();
  ...
  it('GET /api/listings/:id returns blocco siblings for a grouped lot, spanning clusters', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/listings/1030' });
  ...
  it('GET /api/listings/:id 404s for an unknown id', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/listings/999999' });
    expect(res.statusCode).toBe(404);
  });

  it('GET /api/listings/:id resolves OMI context for a listing with a comune reference', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/listings/1030' }); // comune: Roma
    const body = res.json();
  ...
  it('GET /api/listings/:id has no OMI context for a corporate listing', async () => {
    const res = await app.inject({ method: 'GET', url: '/api/listings/2001' });
